...
def filter_by_state(banking_operation: List[Dict[str, str]], state="EXECUTED") -> List[Dict[str, str]]:
...
def sort_by_date(banking_operation: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    return sorted(banking_operation, key=lambda x: datetime.fromisoformat(x["date"]), reverse=False)
